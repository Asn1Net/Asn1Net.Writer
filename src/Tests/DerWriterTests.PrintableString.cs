...
namespace Net.Asn1.Writer.Tests
{
    ...
    {
        [TestCase("13 04 61 62 63 64", "abcd")]
        [Category("PRINTABLE STRING")]
        [Test]
        public void WritePrintableString(string correctResultHex, string valueToTest)
        {
            var encoded = Helpers.GetExampleBytes(correctResultHex);
            using (var ms = new MemoryStream())
            {
                var asn1Obj = new Asn1PrintableString(valueToTest);
